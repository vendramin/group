...
If $f\colon G\to H$ is a group homomorphism, then $G/\ker f\simeq f(G)$.
\end{theorem}
...
\begin{exercise}
Prove that $\Q/\Z\simeq\bigcup_{n\geq1}G_n$.
\end{exercise}
...
\begin{example}
\label{exa:Z24}
Let $G=\Z/24$, $H=\langle 4\rangle$ and $N=\langle 6\rangle$. Since $G$ is abelian, $H$ and $N$ are normal in $G$. Then 
...
\begin{xca}
Prove that if $G$ is a meta-abelian group and $H$ is a subgroup of
...
The following theorem is known as the Correspondence Theorem.
It is both powerful and fundamental. To understand how it applies 
to a group homomorphism
$f\colon G\to f(G)$ with kernel $K=\ker f$, it is useful 
to have in mind the following diagram: 
\[
\begin{tikzcd}
        && G \\
        & L && {f(G)} \\
...
\begin{proposition}
    If $f\colon G\to G_1$ is a group homomorphism 
...
Let us show that $\alpha$ is well-defined. Assume that 
$gH=g_1H$. Then $g_1^{-1}g\in H$ and 
\[
  f(g_1)^{-1}f(g)=f(g_1^{-1}g)\in f(H).
\]
This implies that $f(g_1)f(H)=f(g)f(H)$. 
...
\]
for some $h\in H$. 
This implies that $g_1=ghk$ for some $k\in\ker f\subseteq H$ and hence 
...
One way to verify that every subgroup of  $Q_8$ is normal is to list all its subgroups 
and check their normality individually. The subgroups of $Q_8$ are $\{1\}$, $\{1,-1\}$, $\{1,-1,i,-i\}$, $\{1,-1,j,-j\}$, $\{1,-1,k,-k\}$ and $Q_8$. 

Let us try a different approach. Let $N=\{-1,1\}$. 
Then $N$ is normal in $Q_8$, as $N\subseteq Z(Q_8)$. Since $|Q_8/N|=4$, 
...
% Alternative: 
%> { IsNormal(G,S`subgroup) : S in Subgroups(G) };
%{ true }
...
\[
G\simeq\langle (12)(34)\rangle\subseteq\Alt_4.
\]
Assume 
that $|G|>2$.
Let $f\colon G\to\Sym_n$ be the injective group homomorphism
...
\begin{optional}
We show that in a group no product needs parentheses. By Cayley’s theorem, a group  $G$  is isomorphic to a subgroup of $\Sym_G$. The group operation is then just the composition of certain maps. And the composition of maps is an associative operation, that is no composition of finitely many maps requires parentheses: 
\[

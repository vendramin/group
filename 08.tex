...
\lecture\label{lec:semidirect}

\index{Exact factorization of groups}
We say that a group $G$ admits an \emph{exact factorization} through 
...
Let us briefly discuss \emph{direct products}. Let $G$ be a group and $H$ and $K$ be normal subgroups with trivial intersection, that is $H\cap K=\{1\}$. 
...
By Exercise \ref{xca:direct_product}, 
...
\index{Group!of affine transformations}
    ...
    of $G$, and is isomorphic to the additive group $\R$. In fact, let $f(x)=x+b$ be an element of $K$ 
...
We say that a group $G$ is a \emph{semidirect product} of $Q$ and $K$ if $K$ 
is normal in $G$ and 
...
$g=x_1y_1$ with $x_1\in K$ and $y_1\in Q$. Then $x_1^{-1}x=y_1y^{-1}\in K\cap Q=\{1\}$ 
...
Let us discuss how groups act on groups. An action of a group on a group is 
...
We can identify $K\rtimes\{1\}$ with $K$ 
...
\begin{exercise}
Prove that if $G$ is a semidirect product of the normal subgroup 
...
\begin{example}
    ...
    Hence one can construct the semidirect product $K\rtimes Q$. The operation of 
    this group is 
    \[ 
        (d,c)(b,a)=(cb+d,ca). 
    \]
...
one proves that $\Alt_n$ cannot be written as a semidirect product of proper subgroups. 
...
\]
the semidirect product $T=K\rtimes_\tau Q$ is a non-abelian group of order 12. Moreover,
$T\not\simeq\Alt
...
\begin{exercise}
    ...
    Prove that the group of Exercise~\ref{xca:affine} is a semidirect product. 
\end{exercise}
